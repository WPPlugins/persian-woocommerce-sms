...
class WoocommerceIR_Settings_SMS {
    ...
    function __construct() {
		...
		if ( is_admin() ) {
			...
			add_action( 'ps_woo_sms_form_submit_sms_super_admin_settings',  function() {submit_button();});	
			add_action( 'ps_woo_sms_form_submit_sms_product_admin_settings',  function() {submit_button();});	
			...
			add_filter( 'admin_footer_text', array( $this , 'sms_setting_page_footer' ));
			add_filter( 'update_footer', array( $this , 'sms_setting_page_footer_copyright'), 11);
			...
			add_filter( 'sms_super_admin_settings_settings' , array( $this, 'sms_super_admin_settings_text') );
			add_filter( 'sms_product_admin_settings_settings' , array( $this, 'sms_product_admin_settings_text') );
		}
	...
	function admin_notice_after_update() {
		
		//after update		
		$sms = ps_sms_options( 'enable_sms', 'sms_main_settings', '' );
		$tg = ps_sms_options( 'enable_tg', 'sms_main_settings', '' );
    ...
    function admin_menu() {
		...
		if ( !empty($ps_version) && version_compare( $ps_version, '2.4.9' , '>' ) )
			add_submenu_page( 'persian-wc', 'تنظیمات پیامک', 'تنظیمات پیامک', 'manage_woocommerce', 'persian-woocommerce-sms-pro', array( $this, 'setting_page' ) );
		else
	...
	function sms_super_admin_settings_text( $settings ) {
	...
	function sms_product_admin_settings_text( $settings ) {
	...
	function sms_setting_page_footer ( $text ) {
	...
	function sms_setting_page_footer_copyright($text) {
		if ( isset($_GET['page']) && $_GET['page'] == 'persian-woocommerce-sms-pro') 
